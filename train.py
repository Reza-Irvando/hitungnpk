...
def build_and_train_feature_extractor(base_model_name, X_train_raw, y_train_raw, X_val_raw, y_val_raw):
    # (Salin fungsi build_and_train_feature_extractor dari kode asli Anda ke sini)
    # ... pastikan input_shape, output_dim, dll. menggunakan variabel dari 'config'
    ...
    # ... [KODE FUNGSI build_and_train_feature_extractor SEPENUHNYA DITEMPATKAN DI SINI] ...
    # Pastikan untuk mengganti hardcoded values dengan variabel dari 'config'
    # contoh: epochs=config.COMMON_EPOCHS
    # ...
    # Saya akan menyalinnya untuk kelengkapan
    base_model = None
    ...
    if base_model_name == "VGG16":
        ...
        full_model = Model(inputs=input_tensor, outputs=Dense(output_dim, activation='linear')(feature_output_layer))
        ...
        history = full_model.fit(X_train_raw, y_train_raw, epochs=config.COMMON_EPOCHS, batch_size=config.COMMON_BATCH_SIZE, validation_data=(X_val_raw, y_val_raw), verbose=1)
    ...
    history = full_model.fit(X_train_processed, y_train_raw, epochs=config.COMMON_EPOCHS, batch_size=config.COMMON_BATCH_SIZE, validation_data=(X_val_processed, y_val_raw), verbose=1)
    
    return feature_extractor, preprocess_input_fn, history


def train_and_evaluate_svr_with_tuning(X_train_feats, y_train_tgt, X_test_feats, y_test_tgt, target_name, arch_name):
    # (Salin fungsi train_and_evaluate_svr_with_tuning dari kode asli Anda)
    # ...
    # Saya akan menyalinnya untuk kelengkapan
    print(f"\n--- Training SVR for {target_name} ({arch_name}) with Hyperparameter Tuning ---")
    svr = SVR()
    param_grid = {
        'kernel': ['rbf', 'linear', 'poly'], 'C': [0.1, 1, 10, 100],
        'epsilon': [0.01, 0.1, 0.2], 'gamma': ['scale', 'auto', 0.01, 0.1]
    }
...
def main():
    ...
    # (Salin kode plot dari bagian akhir kode asli Anda ke sini)
    # ...
    # Plotting Loss
    ...
    if all_models_mape:
        ...
        # --- KODE TAMBAHAN UNTUK MENAMPILKAN ANGKA PADA GRAFIK ---
        # Loop melalui setiap 'container' (setiap set batang untuk N, P, dan K)
        for container in ax.containers:
            # Tambahkan label pada setiap batang di dalam container
