"""
Skrip utama untuk melatih feature extractor ResNet50 dan model SVR
...
# Perubahan 1: Impor ResNet50 dan preprocessor-nya
...
def build_and_train_resnet50(X_train, y_train, X_val, y_val, params):
    """Membangun dan melatih model ResNet50 berdasarkan hyperparameter yang diberikan."""
    ...
    # Perubahan 2: Judul di log konsol
    print(f"\n--- Membangun ResNet50: {run_name} ---")

    input_shape = (config.IMAGE_HEIGHT, config.IMAGE_WIDTH, config.NUM_CHANNELS)
    output_dim = 3 # N, P, K

    # Perubahan 3: Muat model ResNet50 pre-trained
    ...
    # 3. Tambahkan layer custom di atas ResNet50
    x = base_model.output
    ...
    # Perubahan 4: Gunakan preprocessor khusus ResNet50
    X_train_prep = resnet50_preprocess(X_train * 255.0)
    X_val_prep = resnet50_preprocess(X_val * 255.0)
...
def main():
    ...
    print(f"Memulai eksperimen dengan ResNet50 dan memvariasikan: '{hyperparameter_to_tune}'")
    ...
    for params in hyperparameter_grid:
        # Perubahan 6: Buat nama unik untuk setiap run eksperimen dengan ResNet50
        run_name = f"ResNet50_E{params['epochs']}_B{params['batch_size']}_LR{params['lr']}_D{params['dropout']}"
        params['run_name'] = run_name
        
        print(f"\n{'='*70}\n--- Memulai Eksperimen: {run_name} ---\n{'='*70}\n")
        
        # Perubahan 7: Panggil fungsi build_and_train_resnet50
        cnn_extractor, history = build_and_train_resnet50(X_train, y_train, X_val, y_val, params)
        ...
        # Ekstrak fitur
        # Perubahan 8: Gunakan preprocessor ResNet50 untuk semua set data
        X_train_prep = resnet50_preprocess(X_train * 255.0)
        X_test_prep = resnet50_preprocess(X_test * 255.0)
    ...
    if all_runs_mape:
        ...
        plt.title(f'Perbandingan MAPE ResNet50 dengan Variasi {hyperparameter_to_tune.upper()}')
        plt.ylabel('MAPE (%)')
        ...
        plt.savefig(os.path.join(config.MODEL_SAVE_DIR, f"mape_comparison_resnet50_tune_{hyperparameter_to_tune}.png"))
        plt.show()
