"""
Skrip utama untuk melatih feature extractor InceptionV3 dan model SVR
...
# Perubahan 1: Impor InceptionV3 dan preprocessor-nya
...
def build_and_train_inceptionv3(X_train, y_train, X_val, y_val, params):
    """Membangun dan melatih model InceptionV3 berdasarkan hyperparameter yang diberikan."""
    ...
    # Judul di log konsol
    print(f"\n--- Membangun InceptionV3: {run_name} ---")

    # InceptionV3 memerlukan ukuran input minimal 75x75
    if config.IMAGE_HEIGHT < 75 or config.IMAGE_WIDTH < 75:
        raise ValueError("InceptionV3 requires input images to be at least 75x75.")
        
    input_shape = (config.IMAGE_HEIGHT, config.IMAGE_WIDTH, config.NUM_CHANNELS)
    output_dim = 3 # N, P, K

    # Muat model InceptionV3 pre-trained
    base_model = InceptionV3(weights='imagenet', include_top=False, input_shape=input_shape)

    # Bekukan bobot dari base_model agar tidak ikut terlatih (transfer learning)
    for layer in base_model.layers:
        layer.trainable = False

    # Tambahkan layer custom di atas InceptionV3
    x = base_model.output
    ...
    # Gunakan preprocessor khusus InceptionV3. 
    # Normalisasi InceptionV3 mengubah piksel dari [0, 255] ke [-1, 1].
    X_train_prep = inceptionv3_preprocess(X_train * 255.0)
    X_val_prep = inceptionv3_preprocess(X_val * 255.0)
...
def main():
    ...
    # Perubahan 5: Judul di log konsol
    print(f"Memulai eksperimen dengan InceptionV3 dan memvariasikan: '{hyperparameter_to_tune}'")
    ...
    for params in hyperparameter_grid:
        # Perubahan 6: Buat nama unik untuk setiap run eksperimen dengan InceptionV3
        run_name = f"InceptionV3_E{params['epochs']}_B{params['batch_size']}_LR{params['lr']}_D{params['dropout']}"
        params['run_name'] = run_name
        
        print(f"\n{'='*70}\n--- Memulai Eksperimen: {run_name} ---\n{'='*70}\n")
        
        # Perubahan 7: Panggil fungsi build_and_train_inceptionv3
        cnn_extractor, history = build_and_train_inceptionv3(X_train, y_train, X_val, y_val, params)
        ...
        # Ekstrak fitur
        # Perubahan 8: Gunakan preprocessor InceptionV3 untuk semua set data
        X_train_prep = inceptionv3_preprocess(X_train * 255.0)
        X_test_prep = inceptionv3_preprocess(X_test * 255.0)
    ...
    if all_runs_mape:
        ...
        # Perubahan 9: Judul plot
        plt.title(f'Perbandingan MAPE InceptionV3 dengan Variasi {hyperparameter_to_tune.upper()}')
        plt.ylabel('MAPE (%)')
        ...
        plt.tight_layout()
        # Perubahan 10: Nama file plot
        plt.savefig(os.path.join(config.MODEL_SAVE_DIR, f"mape_comparison_inceptionv3_tune_{hyperparameter_to_tune}.png"))
        plt.show()
